...
```
# Plan 모드에서의 프롬프트 예시
Todo 앱의 데이터베이스 스키마를 어떻게 구성하면 좋을까?
카테고리 관리를 추가하려면 어떤 파일을 변경해야 해?
```
...
{% hint style="info" %}
`/rename` 명령으로 세션에 설명적인 이름을 지정할 수 있습니다. 예를 들어 `"db-setup"`, `"schema-design"` 등으로 이름을 지정하면 나중에 `--resume`으로 세션을 찾기 쉽습니다.
{% endhint %}
...
```
app/layout.tsx의 타이틀을 "Todo App"으로 변경해줘.
```
...
다음 섹션에서는 [PostgreSQL을 설치 및 설정](05-auth-setup.md)합니다.
