...
컨텍스트 윈도우는 Claude가 한 번에 "기억"할 수 있는 정보의 총량입니다. 여기에는 다음이 모두 포함됩니다:
...
```
서브에이전트를 사용해서 Clerk middleware가 어떻게 동작하는지,
현재 어떤 라우트가 보호되고 있는지 조사해줘.
```
...
학습 트래커 프로젝트에서의 실제 워크플로 예시:

```
# 1. 인증 설정 작업 시작
Clerk를 설치하고 인증을 설정해줘...

# 2. 인증 작업 완료 후 컨텍스트 초기화
/clear

# 3. 데이터베이스 작업 시작 (깨끗한 컨텍스트에서)
Neon Postgres와 Drizzle ORM을 설정해줘...

# 4. 데이터베이스 작업 완료 후 컨텍스트 초기화
/clear

# 5. 스키마 설계 시작
학습 트래커의 데이터베이스 스키마를 설계해줘...
```
