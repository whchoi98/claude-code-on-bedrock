...
### Step 2: 결과 관찰 - 무엇이 문제인가?

Claude가 생성한 코드를 살펴봅니다. 다음과 같은 문제점들을 확인해보세요:
...
Claude Code는 매우 유능한 AI 코딩 어시스턴트이지만, 프로젝트의 컨벤션과 규칙을 알려주지 않으면 **일반적인 패턴을 추측**하여 코드를 생성합니다. 이는 다음과 같은 결과를 초래합니다:

- 프로젝트 내에서 **일관성 없는 코드 패턴**
- 의도하지 않은 **아키텍처 결정**
- 나중에 **수정해야 할 기술 부채**
{% endhint %}
...
문서 없이 코드를 생성했을 때의 문제점을 확인했습니다. 다음 섹션에서는 CLAUDE.md와 가이드 문서를 작성하여 **일관되고 품질 높은 코드**를 생성하는 방법을 배웁니다.
