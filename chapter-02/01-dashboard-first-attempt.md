...
### Step 1: 방법 A — 프롬프트만으로 할 일 목록 페이지 생성

CLAUDE.md나 가이드 문서를 만들지 않은 상태에서, 바로 할 일 목록 페이지 생성을 요청해봅니다.

Claude Code 터미널에서 다음과 같이 입력합니다:

→ *이 프롬프트는 CLAUDE.md 없이 최소한의 지시만으로 할 일 목록 페이지를 생성하기 위한 것입니다:*

```
/todos에 할 일 목록 페이지를 만들어줘:
- 전체 할 일 목록
- 완료/미완료 상태 표시
- 새 할 일 생성 버튼
...
```typescript
// 카테고리와의 관계를 고려하지 않고 단순 조회
const todos = await db.select().from(todosTable);
// 카테고리 정보(이름, 색상)가 누락됨
```

- 카테고리와의 JOIN이 빠질 수 있음
- 우선순위, 마감일 등 필드를 제대로 활용하지 않을 수 있음
...
const allTodos = await db.execute(sql`SELECT * FROM todos`);
```

- Drizzle의 query builder를 제대로 활용하지 않을 수 있음
- 정렬이나 필터링 로직이 빠질 수 있음
...
| 데이터 관계 누락 | 카테고리 JOIN 등 관계 자동 반영 |
...
되돌아갈 시점을 선택하라는 프롬프트가 나타나면, 할 일 목록 페이지 생성 요청 **이전** 시점을 선택합니다.
