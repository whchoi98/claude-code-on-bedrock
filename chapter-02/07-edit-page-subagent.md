...
> 🎯 **이 섹션에서 배울 Claude Code 기능**: 서브에이전트 (탐색 위임, 코드 리뷰)
...
프로젝트가 커지면 코드를 탐색하는 것만으로도 컨텍스트가 가득 찰 수 있습니다. 서브에이전트를 사용하면:

- **탐색은 별도로**: 서브에이전트가 파일을 읽고 분석
- **구현은 깨끗한 컨텍스트에서**: 메인 대화는 요약된 결과만 받아 깨끗하게 유지

> 💡 **비유**: 서브에이전트는 "조수에게 자료 조사를 시키고, 결과 요약만 받는 것"과 같습니다. 조수가 도서관에서 10권의 책을 읽고, 핵심 내용을 한 페이지로 정리해 주는 것처럼, 서브에이전트는 코드베이스를 탐색하고 결과만 돌려줍니다.
...
→ *이 프롬프트는 서브에이전트에게 기존 코드 패턴을 조사시켜, 편집 페이지를 일관되게 만들 수 있는 정보를 수집하기 위한 것입니다:*
...
→ *이 프롬프트는 편집 기능의 기반이 되는 세션 상세 페이지를 생성하기 위한 것입니다:*
...
→ *이 프롬프트는 서브에이전트가 분석한 기존 패턴을 따라, 일관된 편집 페이지를 구현하기 위한 것입니다:*
...
→ *이 프롬프트는 서브에이전트에게 코드 리뷰를 위임하여, 보안/일관성/접근성 이슈를 발견하기 위한 것입니다:*
...
→ *이 프롬프트는 서브에이전트의 코드 리뷰에서 발견된 구체적인 이슈를 수정하기 위한 것입니다:*
...
→ *이 프롬프트는 이전에 만든 커밋 스킬로 편집 페이지 변경사항을 커밋하기 위한 것입니다:*
...
- [ ] 서브에이전트로 기존 코드 패턴 분석 완료
- [ ] 세션 상세 및 편집 페이지 구현 완료
- [ ] 서브에이전트 코드 리뷰로 이슈 발견 및 수정 완료
