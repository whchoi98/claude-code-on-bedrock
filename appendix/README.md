...
CLAUDE.md가 너무 길거나 모호하지 않은지 확인하세요.

- CLAUDE.md는 **구체적이고 간결**할수록 좋습니다
- 모호한 지시(예: "코드를 잘 작성하세요") 대신 명확한 규칙(예: "서버 컴포넌트를 기본으로 사용")을 작성하세요
- 너무 많은 규칙은 오히려 효과를 떨어뜨립니다 — 핵심 규칙 위주로 유지하세요

#### "같은 에러가 반복돼요"

컨텍스트가 오염되었을 수 있습니다. `/clear`로 초기화한 후 더 구체적인 프롬프트로 다시 시도하세요.
...
대화가 길어져 컨텍스트가 가득 찬 것입니다. 다음 방법을 시도하세요:

1. `/compact`로 대화를 요약하여 공간 확보
2. `/clear` 후 필요한 컨텍스트만 다시 제공
3. 탐색이 많이 필요한 작업은 **서브에이전트**에 위임하여 메인 컨텍스트 보호

#### "Claude가 잘못된 패턴으로 코드를 생성해요"

CLAUDE.md에 올바른 패턴을 명시했는지 확인하세요. 예를 들어:

```markdown
# CLAUDE.md에 추가
## 반드시 지켜야 할 규칙
- 데이터 패칭은 반드시 서버 컴포넌트에서 수행 (useEffect 사용 금지)
- 모든 데이터베이스 쿼리는 lib/queries/ 디렉토리의 함수를 통해 수행
- UI 컴포넌트는 반드시 shadcn/ui 사용
```
