...
> 🎯 **이 섹션에서 배울 Claude Code 기능**: CLI 파이프 (claude -p), Hooks, 비대화형 모드
...
→ *이 프롬프트는 빌드를 실행하고 에러가 있으면 자동으로 수정하기 위한 것입니다:*
...
→ *이 프롬프트는 린트를 실행하고 발견된 문제를 자동 수정하기 위한 것입니다:*
...
→ *이 프롬프트는 Git 히스토리를 확인하고 요약을 받기 위한 것입니다:*
...
→ *이 프롬프트는 패키지 취약점을 검사하고 설명을 받기 위한 것입니다:*
...
→ *이 프롬프트는 테스트를 실행하고 실패한 테스트를 자동 수정하기 위한 것입니다:*
...
→ *이 프롬프트는 빌드 에러 출력을 파이프로 Claude에 전달하여 수정 방법을 받기 위한 것입니다:*
...
→ *이 프롬프트는 에러 로그 파일을 Claude에 전달하여 분석 및 수정 제안을 받기 위한 것입니다:*
...
→ *이 프롬프트는 Git diff의 변경 파일 목록을 Claude에 전달하여 코드 리뷰를 받기 위한 것입니다:*
...
→ *이 프롬프트는 테스트 실패 출력을 Claude에 전달하여 원인 분석을 받기 위한 것입니다:*
...
→ *이 프롬프트는 DB 스키마 파일을 Claude에 전달하여 분석 및 개선 제안을 받기 위한 것입니다:*
...
→ *이 프롬프트는 비대화형 모드로 프로젝트에 대한 간단한 질문을 하기 위한 것입니다:*
...
→ *이 프롬프트는 스테이징된 변경사항을 기반으로 커밋 메시지를 자동 생성하기 위한 것입니다:*
...
→ *이 프롬프트는 브랜치의 커밋 이력을 기반으로 PR 설명을 자동 생성하기 위한 것입니다:*
...
→ *이 프롬프트는 파일 수정 후 자동으로 린트를 실행하는 Hook을 생성하기 위한 것입니다:*
...
- [ ] `claude -p` 명령 실행 확인 (비대화형 모드)
- [ ] 파이프를 통해 다른 도구의 출력을 Claude에 전달 확인
- [ ] Hooks 설정 확인 (`.claude/settings.json`에 Hook 정의)
- [ ] `/hooks` 명령으로 Hook 관리 인터페이스 접근 확인
